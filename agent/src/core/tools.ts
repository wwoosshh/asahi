...
export type ToolCtx = {
  repos: { memories: MemoriesRepo; users: UsersRepo };
  role: Role;
  isPrivate: boolean;
  isOwner: boolean;
  userId: string;
...
// ── 턴별 도구셋(능력 계층, §7.1) ────────────────────────────────────────────
// owner-DM → 파일 도구 + 기억 + 접근관리. 손님 DM → 기억(본인)만. 서버 → recall(공용)만.
export function allowedToolsFor(role: Role, isPrivate: boolean, isOwner: boolean): string[] {
  if (isOwner && isPrivate) return [...FILE_TOOLS, t("remember"), t("recall"), t("manage_access")];
