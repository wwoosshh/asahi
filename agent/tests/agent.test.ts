...
import { allowDirHandler } from "../src/core/tools.js";
...
async function repos(): Promise<ToolRepos> {
  ...
  return { memories: new MemoriesRepo(db), users: new UsersRepo(db), allowedDirs: new AllowedDirsRepo(db) };
}
...
describe("buildToolCtx — makeRunAgentTurn 의 ToolCtx 구성(리뷰 #1 회귀)", () => {
  it("ownWorkstation 을 포함해 TurnContext 의 모든 필드를 ToolCtx 로 그대로 복사한다", async () => {
    ...
    const ctx = buildToolCtx(r, context);
